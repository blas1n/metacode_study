...
- **VectorRAG** = pgvector — `litellm` 임베딩 (bge-m3 via Ollama, 1024d) + SQLAlchemy/asyncpg raw SQL (bsvibe `backend/embedding`)
...
- 초기 빌드는 루브릭 예시인 `text-embedding-3-small`(1536d) 로 잡았으나 KO 질의↔EN 문서
  cross-lingual 갭으로 top-1 코사인 평균이 0.401 에 머물러, 멀티링구얼 `bge-m3`(1024d, Ollama)
  로 교체했습니다 → top-1 코사인 평균 0.590, Hit@1 93.3%, MRR 0.956. bsvibe-app prod 도 Ollama
  임베딩 스택을 쓰므로 컨벤션 일치.
