...
| 단계 | 무엇이 일어나나 |
|---|---|
| **① 던지세요** | 어디서든 ⌘K 또는 `+ Direct` 한 줄. 메일·GitHub 이슈가 들어오면 자리에 없어도 시작 |
...
- **진행 중** — 지금 돌고 있는 작업
- **결정 대기** — 갈림길에서 올라온 안건, 그리고 밖으로 내보내기 전 승인을 기다리는 일
- **지난 작업** — 끝난 일과 결과물이 시간 순서대로
...
모든 결과는 한 장 짜리 **결과 리포트** 로 도착 (다섯 블록):
...
<div class="card"><h3>안전 가드레일</h3><p>Safe Mode · 검증 약속 · 작업 공간별 권한 분리.</p></div>
...
> `Inside` 화면에서 학습한 그래프를 직접 봅니다. 잘못 학습한 노드는 한 클릭으로 되돌리기 — 30 초 안에 취소 가능.
...
1. **Brief** — 진행 중 / 결정 대기 / 지난 작업 한 화면
2. **한 줄 던지기** — ⌘K 입력창에 자연어로
3. **결과 리포트** — 요청 → 만든 것 → 어떻게 확인했나 → 변경 코드
4. **Inside** — 학습된 지식 그래프 시각화
5. **되돌리기** — 잘못 학습한 노드 한 클릭 → 30 초 취소 토스트
